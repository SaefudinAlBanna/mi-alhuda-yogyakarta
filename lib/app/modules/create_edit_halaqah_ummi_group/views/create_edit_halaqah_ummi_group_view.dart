...
class CreateEditHalaqahUmmiGroupView extends GetView<CreateEditHalaqahUmmiGroupController> {
  const CreateEditHalaqahUmmiGroupView({Key? key}) : super(key: key);
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // [PENAMBAHAN DI SINI] Menambahkan TextField untuk Lokasi
          ...
          const Text("Pilih Fase Grup", style: TextStyle(fontWeight: FontWeight.bold)),
          Obx(() => Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: ["A", "B", "C"].map((fase) => Padding(
              padding: const EdgeInsets.symmetric(horizontal: 8.0),
              child: ChoiceChip(
                label: Text("Fase $fase"),
                selected: controller.selectedFase.value == fase,
                onSelected: (selected) {
          ...
          Obx(() => DropdownButtonFormField<PegawaiSimpleModel>(
            ...
            isExpanded: true, 
            items: controller.availablePengampu.map((p) {
              return DropdownMenuItem<PegawaiSimpleModel>(value: p, child: Text(p.nama, overflow: TextOverflow.ellipsis));
            }).toList(),
            ...
            hint: const Text("Pilih pengampu yang tersedia"),
          )),
        ],
      ),
    );
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            // [PERUBAHAN #1] Tampilkan progresi saat ini di subtitle
            subtitle: Text(
              "Kelas: ${siswa.kelasId.split('-').first} • Progres: ${siswa.tingkat} ${siswa.detailTingkat}",
              style: const TextStyle(fontSize: 12),
            ),
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                // [PERUBAHAN #2] Tombol Edit Progres
                IconButton(
                  icon: const Icon(Icons.edit_note, color: Colors.blue),
                  onPressed: () => controller.showEditProgresDialog(siswa),
                  tooltip: "Edit Progres Siswa",
                ),
                // Tombol Hapus Siswa dari Grup
                IconButton(
